...
-- 2
SELECT MARK, COUNT(*) AS COUNT_THIS_MARK, (SELECT COUNT(*) FROM AUTO WHERE MARK != A.MARK) AS COUNT_OTHER_MARK
FROM AUTO A GROUP BY MARK;
...
SELECT a.* FROM test_a a LEFT JOIN test_b b ON a.id = b.id
WHERE b.id IS NULL;